...
const {snils, oid, randomDate, ogrn, kpp,
    oms, passport, birthSert, innPerson, innOrg } = require("./ids");
...
function rand(cnt, finish) {
    ...
    if (Array.isArray(cnt)) {
        ...
        let maxSumm=0; for(let i=0; i<cnt.length; i++) maxSumm+=cnt[i].num;
        const val=Math.random()*(maxSumm-1);
        let s=cnt[0].num;
        let i=0;
        while (val>s && i<cnt.length) { i++; s+=cnt[i].num }
...
module.exports = {Factory, Generators, anyOrg, anyPerson: genOnePerson, genOnePerson, rand, snils, oid, randomDate, ogrn, kpp, oms, passport, birthSert, innPerson, innOrg, address };
